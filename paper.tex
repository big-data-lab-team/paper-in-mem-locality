...
%\todo{Lazy evaluation should be mentioned here, or elsewhere in the paragraph.
...
% We haven't benchmarked shuffles but we know it's important.
...
% When is it useful to use a Big Data engine for neuroinformatics applications?

% Future work:
% - now that we know that Spark brings performance to neuro applications, make it usable
% - scheduling in a shared environment.
% - benchmark a large scale fmri processing to confirm our speculations
% - try in-memory distributed file systems such as Apache Ignite
% - workflow-aware cache eviction strategies instead of LRU/n
% - improve Spark to better use page caches
