...
\begin{abstract}
    ...
    However, current neuroimaging data engines have not adopted any strategies
    to address the rise
    in neuroimaging data. Here, we 
    ...
    application to the filesystem-application concurrent write rate. 
...
operations performed on images of size larger than 75GB. 
...
In Big Data applications, there is a significant cost associated with data movement.
Big Data processing engines have addressed these issues by diminishing the amount 
of data movement that occurs during the execution of an application. The 
MapReduce~\cite{dean2008mapreduce} framework popularized the concept of locality-aware
scheduling, which ensured tasks were scheduled nearest to where the data the data was
located (data locality), thereby reducing the number of network transfers in an application.
However, it was found that performance in MapReduce could be improved upon by also
reducing the disk I/O between tasks by keeping data in-memory whenever possible,
and thus, in-memory computing was added to the Apache Spark~\cite{zaharia2016apache} framework.
Frameworks like Apache Spark also included the lazy evaluation strategy, which performs computations
only when invoked. This allows for additional optimizations such as the regrouping of tasks
and only invoking what is absolutely necessary to obtain the desired result.

Frameworks such as
MapReduce and Spark have become mainstream tools for data analytics, however, many
...
fit in memory. Spark will spill any RDD elements that do not fit in memory to disk.
...
\end{equation}
This defines a data-write rate beyond which the page cache becomes 
...
\subsection{Infrastructure} % 1/4 a page (half a col)
 ...
 caching effects (measure\_bandwidth.py script).
...
in neuroscience. The other being the processing large MRI datasets, 
...
experiment, the data-write rate was kept static at 178.6MB/s, however, this 
...
the data-write rate varied. Once again, we ensured that the data-write rate fell in 
multiple different regions of the inequality. The data-write rate ranged from 
...
\begin{subfigure}{\columnwidth}
    \centering
    \begin{subfigure}{\columnwidth}
        \includegraphics[clip,width=\columnwidth]{results/figures/s-iterations.pdf}%
    \end{subfigure}
    \begin{subfigure}{\columnwidth}
        \includegraphics[clip,width=\columnwidth]{results/figures/s-iterations.pdf}%
    \end{subfigure}
...
speed-up provided by in-memory computing was larger than 1 for data-write 
rates larger than $\delta/\gamma$ (local disk) or $\Delta/\Gamma$ 
(Lustre). Conversely, the speed-up provided by in-memory computing 
remained close to 1 for data-write rates smaller than $\delta/\gamma$ (local 
...
size), the data-write rate would be 1.8MB/s, which would reach the 
...
As for the fMRI speculation, the data-write 
