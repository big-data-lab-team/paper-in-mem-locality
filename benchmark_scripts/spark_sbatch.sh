...
MEM_PER_EXEC=$(echo $MEM_PER_EXEC - $HEAP_OVERHEAD | bc)
MEM_PER_EXEC=$(echo $MEM_PER_EXEC/1 | bc)
...
srun -n 1 -N 1 $SPARK_HOME/bin/spark-submit --master=${MASTER_URL} --num-executors=${NEXECUTORS} \
    --executor-memory=${MEM_PER_EXEC}M --driver-memory=${MEM_PER_EXEC}M --conf spark.locality.wait=600s \
    --conf spark.default.parallelism=$parallelism $spscript
