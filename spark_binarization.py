...
def binarize_data(filename, data, metadata, threshold, benchmark, start, output_dir):
    start_time = time() - start
...
def main():
    ...
    imRDD = sc.binaryFiles('file://' + os.path.abspath(args.bb_dir)) \
              .map(lambda x: read_img(x[0], x[1], args.benchmark, start, args.output_dir))

    for i in range(args.iterations):
        imRDD = imRDD.map(lambda x: binarize_data(x[0], x[1], x[2], threshold, 
            args.benchmark, start, args.output_dir))


    imRDD.map(lambda x: save_binarized(x[0], x[1], x[2], args.benchmark, start, args.output_dir)) \
         .collect()
